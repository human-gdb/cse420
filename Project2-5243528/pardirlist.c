...
void fileWriter(SDLL*list,FILE* fp){
    ...
    Node*node=list->head;
    while(node != NULL){
        if(runMode == PARALLEL) pthread_join(node->item->tid,NULL);
        fprintf(fp,"%d:%d:%d:%s\n",node->item->level,node->item->num,node->item->count,node->item->path);
        node=node->next;
    }
    fclose(fp);
...
int main(int argc, char *argv[]){
    ...
    if(argc < 5){
    	perror("Please enter commandline arguments as such:/n./pardirlist absolute_path keyword output_file ispar");
